...
class Skipganomaly(BaseModel):
    ...
    def __init__(self, opt, data=None):
        ...
        self.noise = torch.empty(size=(self.opt.batchsize, self.opt.nc, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device)
    ...
    def backward_d(self):
        ...
        self.err_d_lat = self.err_g_lat.detach()

        # Combine losses.
        self.err_d = self.err_d_real + self.err_d_fake + self.err_d_lat
